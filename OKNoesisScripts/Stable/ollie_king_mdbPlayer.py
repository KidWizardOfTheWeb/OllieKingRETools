...
# Will try to add noewin GUI in the future
# import os
# from ctypes import *
# import noewin
...
def bcLoadModel(data, mdlList):
    ...
    # We use this section below to check whether a player model has an alt color, using the second alt for reference. If not, their default texture is loaded. If true, the user is prompted to select a number 1-4 to select an alt.
    txb_fileAlt = noesis.fileIsLoadable(curr_folder + mesh_name + "02.txb")
    txb_file = ""

    if txb_fileAlt == True:
        altColorPrompt = noesis.userPrompt(noesis.NOEUSERVAL_INT, "Alternate colors", "Type 1, 2, 3 or 4 to select an alternate color.", "1")
        if altColorPrompt == 1:
            txb_file = curr_folder + mesh_name + ".txb"
        elif altColorPrompt == 2:
            txb_file = curr_folder + mesh_name + "02.txb"
        elif altColorPrompt == 3:
            txb_file = curr_folder + mesh_name + "03.txb"
        elif altColorPrompt == 4:
            txb_file = curr_folder + mesh_name + "04.txb"
        else:
            txb_file = curr_folder + mesh_name + ".txb"
    else:
        txb_file = curr_folder + mesh_name + ".txb"
    ...
    # dialogue box stuff to add later?
    # txb_file = curr_folder + mesh_name + X + ".txb"
    # X being the alterate color number, 01-04.
    # if option invoked:
    #   if rapi.checkFileExists(txb_file):
    #     tex_file = NoeBitStream(rapi.loadIntoByteArray(txb_file))
    #     mat_list, tex_list = ReadTextures(tex_file)
    ...
    #     mat_list = []
    #     tex_list = []
    # sets model materials after, loads new texture possibly.
